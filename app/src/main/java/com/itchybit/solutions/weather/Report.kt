...
data class Sys(
		val type: Int, val id: Int, val message: Double, val country: String, val sunrise: Long,
		val sunset: Long,
)

data class Report(
		val coord: Coord, val weather: List<Weather>, val base: String, val main: Main,
		val visibility: Int?, val wind: Wind, val clouds: Clouds, val rain: Rain?, val snow: Snow?,
		val dt: Long, val sys: Sys, val timezone: Int, val id: Long, val name: String, val cod: Int,
)
